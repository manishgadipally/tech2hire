function searchBtnClick() {
  ...
  var results = GetFromCache(filter);
  if (results) {
    FilterRows(filter, results);
  } else {
    var res = search();
    AddToCache(filter, res);
    FilterRows(filter, res);
  }
}

function FilterRows(filter, results) {
...
function AddToCache(filter, results) {
  sessionStorage.setItem(filter, results);
}
